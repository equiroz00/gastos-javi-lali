...
**22/08/2026 — Dos variantes de interfaz conviviendo, elegibles por usuario (Claude)**
- **Qué se hizo:** las dos direcciones del rediseño ("Estado de cuenta" y "Panel de trabajo") conviven en la app y cada uno elige la suya en Config › Apariencia. La preferencia va a `userPreferences/{usuario}.ui`, el mismo carril que `theme` y `font`: **no afecta a la otra persona**.
- **Decisión de arquitectura — variante y tema son ejes ortogonales.** Los mockups de las dos direcciones usan *la misma paleta* (`#3D2F73` es el `navy` de Budget Flow): lo que cambia es el layout, no el color. Así que `UI_VARIANTS` en `constants.ts` contiene **solo tokens de estructura** (ancho del riel, tarjeta vs. filete, radios, densidad, estilo de pasos y de nav) y el color sigue saliendo de `C`. Resultado: 3 temas × 2 variantes = 6 combinaciones válidas, y el tema **Original de Lali se preserva** en las dos direcciones.
- **Se evitó escribir la app dos veces.** En vez de duplicar las 7 pantallas, la diferencia se concentró en las primitivas que todas ya usaban: `Card` (tarjeta con borde vs. bloque con filete inferior) y `SectionLabel` (versalita con filete fuerte vs. etiqueta apagada). Cambiar la variante reescribe todas las pantallas sin tocarlas una por una. Se sumaron `Row` y `AmountBand`.
- **Shell por variante** (`App.tsx`): riel de íconos en escritorio — 64px claro en 'cuenta', 56px ink en 'panel' — y nav móvil de píldora flotante ('cuenta') vs. barra al ras ('panel'). El riel "ink" se resuelve por contraste (`C.dark`, token nuevo en `ThemeConfig`) y **no** invirtiendo el color: en el tema Oscuro `navy` es casi blanco y una inversión ingenua dejaba el riel blanco.
- **Pantallas con tratamiento propio:** Inicio (el balance se convierte en la banda de acento con la cifra en JetBrains Mono) y Agregar gasto (barras de progreso vs. pestañas navegables con tilde en los pasos hechos).
- **Sin migración ni deploy de reglas:** `userPreferences/{id}` ya era abierto para la pareja, así que el campo `ui` entra sin tocar `firestore.rules`. Quien nunca eligió cae en 'cuenta' vía `coerceUI`.
- **Huecos del material de diseño** (ver "Pendiente" abajo): el documento **no tiene Inicio diseñado** en ninguna de las dos direcciones (referencia "1b"/"1c", que no están en el proyecto), y de la Dirección B **solo sobrevivió "Agregar gasto"** porque el archivo se cortó en el tope de 256 KiB de la herramienta de lectura. Esas pantallas salen coherentes por las primitivas, pero no están calcadas de un mockup.
- **Pendiente:** conseguir los mockups de Inicio (1b/1c) y la cola de la Dirección B para afinar Stats, Lo mío, Historial, Config y Login de 'panel'. Verificación visual con datos reales y con las 2 cuentas.
