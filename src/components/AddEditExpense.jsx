...
export default function AddEditExpense(props){
  ...
  function blankForm(){
    return {date:todayStr(),description:'',amount:'',category:allCats[0]||DEFAULT_CATS[0],paymentMethod:PAY_METHODS[0],bank:BANKS[0],paidBy:currentUser,responsible:'Ambos',currency:'ARS',customCurrency:''};
  }
  ...
  var javiAmount   = amts.javiAmount; var laliAmount = amts.laliAmount;
  ...
  var splitSummary = (form.paidBy==='Javi'?'👨':'👩')+' '+form.paidBy+' · '+form.responsible;
  ...
  function enqueue(){
    ...
    var item=sanitize(Object.assign({},form,{id:Date.now().toString(),amount:parseFloat(form.amount),javiAmount:javiAmount,laliAmount:laliAmount,currency:finalCur,period:getPeriod(form.date,periods),createdBy:currentUser,createdAt:new Date().toISOString()}),allCats);
    setQueue(function(q){return q.concat([item]);});
    setForm(function(f){return Object.assign({},f,{description:'',amount:''});});
    setErrors({});
  }
  ...
  function submit(){
    ...
    if(Object.keys(e).length){setErrors(e);return;}
    var finalCur=form.currency==='Otra'?(form.customCurrency||'ARS'):form.currency;
    var base=Object.assign({},form,{id:isEditMode?(initialData&&initialData.id)||Date.now().toString():Date.now().toString(),amount:parseFloat(form.amount),javiAmount:javiAmount,laliAmount:laliAmount,currency:finalCur,period:getPeriod(form.date,periods)});
    if(!isEditMode){base.createdBy=currentUser;base.createdAt=new Date().toISOString();}
    if(isEditMode){
      handleEdit(base);
    } else if(useCuotas&&finalCuotas>1){
      handleAddPlan(base,finalCuotas,isRetro?paidNum:0,isRetro?retroStartPer:null);
    } else if(queue.length>0){
      var all=queue.concat([sanitize(Object.assign({},base,{id:Date.now().toString()}),allCats)]);
      handleAddMultiple(all);
    } else {
      handleAdd(base);
    }
  }
  ...
  // ── División section (collapsible) ────────────────────────────────────────
  var divisionSection=React.createElement('div',{style:{marginTop:'0.75rem'}},
    React.createElement('button',{onClick:function(){setSplitOpen(function(o){return !o;});},style:{width:'100%',background:C.bg,border:'1px solid '+C.border,borderRadius:splitOpen?'0.85rem 0.85rem 0 0':'0.85rem',padding:'0.6rem 0.85rem',display:'flex',justifyContent:'space-between',alignItems:'center',cursor:'pointer',fontFamily:F}},
      React.createElement('span',{style:{fontSize:'0.8rem',fontWeight:700,color:C.navy}},'¿Cómo se divide?'),
      React.createElement('span',{style:{fontSize:'0.72rem',color:C.textMuted}},splitOpen?'▲':(splitSummary+' ▼'))
    ),
    splitOpen?React.createElement('div',{style:{background:C.bg,border:'1px solid '+C.border,borderTop:'none',borderRadius:'0 0 0.85rem 0.85rem',padding:'0.75rem'}},
      ...
      showSplit?React.createElement('div',{style:{background:C.surface,borderRadius:'0.75rem',padding:'0.6rem 0.85rem',display:'flex',justifyContent:'space-between',marginTop:'0.75rem',border:'1px solid '+C.border}},
  ...
  var queueSection=queue.length>0?React.createElement('div',{style:{marginTop:'0.75rem',background:C.bg,borderRadius:'0.85rem',border:'1px solid '+C.border,overflow:'hidden'}},
    ...
    queue.map(function(item,i){
      return React.createElement('div',{key:item.id,style:{display:'flex',alignItems:'center',gap:'0.5rem',padding:'0.5rem 0.85rem',borderBottom:i<queue.length-1?'1px solid '+C.border:'none'}},
        React.createElement('div',{style:{fontSize:'1rem'}},(catEm||function(c){return'📦';})(item.category)),
  ...
  var step2=React.createElement('div',null,
    ...
    isEditMode
      ?React.createElement('div',{style:{background:C.bg,borderRadius:'0.85rem',padding:'0.75rem',marginBottom:'0.5rem',border:'1px solid '+C.border}},
          ...
          React.createElement('div',{style:{display:'flex',gap:'0.5rem',marginTop:'0.5rem'}},
            React.createElement('div',{style:{flex:1}},Lbl('Monto'),React.createElement('input',{style:inpStyle(),type:'number',value:form.amount,onChange:function(e){set('amount',e.target.value);},placeholder:'0'})),
    ...
    showSplit?React.createElement('div',{style:{background:C.bg,borderRadius:'1rem',padding:'0.85rem 1rem',display:'flex',justifyContent:'space-between',marginTop:'0.75rem',border:'1px solid '+C.border}},
      React.createElement('div',{style:{textAlign:'center',flex:1}},React.createElement('div',{style:{fontSize:'0.7rem',color:C.textMuted}},'👨 Javi'),React.createElement('div',{style:{fontWeight:800,color:C.navy}},fmt(javiAmount,cur))),
      React.createElement('div',{style:{width:'1px',background:C.border}}),
      React.createElement('div',{style:{textAlign:'center',flex:1}},React.createElement('div',{style:{fontSize:'0.7rem',color:C.textMuted}},'👩 Lali'),React.createElement('div',{style:{fontWeight:800,color:C.accent}},fmt(laliAmount,cur)))
    ):null,
